			...
			if (getThemeConfig.value.isShareTagsView ? v.path === path : v.url === path) {
				...
				setTimeout(() => {
					if (state.tagsViewList.length === k && getThemeConfig.value.isShareTagsView ? state.routePath === path : state.routeActive === path) {
						// 最后一个且高亮时
							console.log("%c [ arr[arr.length - 1] ]", "font-size:13px; background:linear-gradient(to right , #9B63FF, #462188); color:yellow;", arr[arr.length - 1])
...
onBeforeMount(() => {
	...
	mittBus.on('openShareTagsView', () => {
			console.log("%c [ getThemeConfig.value.isShareTagsView ]", "font-size:13px; background:linear-gradient(to right , #9B63FF, #462188); color:yellow;", getThemeConfig.value.isShareTagsView)

		if (getThemeConfig.value.isShareTagsView) {
			// router.push('/home');
			router.push('/adminCenter/tenant');
			state.tagsViewList = [];
			console.log("%c [ state.tagsViewList ]", "font-size:13px; background:linear-gradient(to right , #9B63FF, #462188); color:yellow;", state.tagsViewList)
